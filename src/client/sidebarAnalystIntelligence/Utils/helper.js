...
const convertRowIndexIntoNumber = (rowIndex) => {
  if (
    (typeof rowIndex === 'string' || rowIndex instanceof String) &&
    /^[a-zA-Z]+$/.test(rowIndex)
  ) {
...
export const downloadAndRenderDataOnActiveCell = async (
  ...
  try {
    ...
    await mapSourceTableDataToTheCells(
      tableData,
...
const mapSourceTableDataToTheCells = async (data, startingAddress) => {
  const headerCellNames = (data.columns_meta || [])
  ...
  const mapping = dataSetAccordingToLimit.map(
    (setAccordingToTheLimit, index) => {
      const [row, column] = startingAddress.split(/(\d+)/);

      const cellsLength = (data.columns_meta || []).length - 1;

      const endPoint = convertNumberIntoCharacter(
        convertRowIndexIntoNumber(row) + cellsLength
      );

      const cellRange = `${row}${
        Number(column) + index * LIMIT_TO_UPDATE_CELLS_PER_CALL
      }:${endPoint}${
        Number(column) +
        index * LIMIT_TO_UPDATE_CELLS_PER_CALL +
        setAccordingToTheLimit.length -
        1
      }`;
...
export const getSourceTableListingViaApi = async () => {
  try {
    const url = `${
      ADD_ONS_APIS.V2_API.LOAD_ALL_TABLES_NO_PARENT_FOLDER
    }?query=${getQueryObject()}&details=false`;
 ...
 *
 */
export const getCommentAfterRemovingPrefixAndConvertingTheStringIntoObject = (
  content
) => {
  let mappedContent = {};
    ...
    (mappedData || []).forEach((cell) => {
      ...
      if (Object.keys(content?.downloadTableData || {}).length) {
        pendingPromises.push(
          downloadAndRenderDataOnActiveCell(content, cell.address)
        );
      }
...
export const getSourceTableListingOptions = (list) => {
  return (list || []).map(
    ({ display_name: name = '', id, table_type = '', label = '' }) => {
      return {
        value: id,
        label: name,
        labelName: label,
 ...
 *
 */
export const loadSourceTableDataForParticularId = async (
  id,
  details = false
) => {
  try {
    const res = await httpGet(
      `${ADD_ONS_APIS.V2_API.LOAD_SOURCE_TABLE_DETAILS.replace(
        '#ID#',
        id
...
export const setHeightOfTheDropDownAccordingToScreenSize = () => {
  setTimeout(() => {
    if (
      document.getElementsByClassName(DROPDOWN_TREE_CLASS_FOR_INFINITE_SCROLL)
        .length
    ) {
    ...
    setWidthOfNode(
      document.getElementsByClassName(REACT_TREE_DROPDOWN_CONTENT_CLASS_NAME)
    );
  }, 1);
};
...
const setWidthOfNode = (dropdownOptions) => {
  Array.from(dropdownOptions).forEach((element) => {
    ...
    element.style.width = `${
      (window.innerWidth * REACT_TREE_DROPDOWN_INPUT_WIDTH_PERCENTAGE) / 100
    }px`;
  });
};
...
export const handleReactTreeDropdownWidthOnResizeEvent = () => {
  setWidthOfNode(
    document.getElementsByClassName(REACT_TREE_DROPDOWN_INPUT_CLASS_NAME)
  );
